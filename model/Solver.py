...
def solver(X_train, y_train, X_val, y_val, reg, optimizer, lr_decay=1.0, num_iter=100, batch_size=200, verbose=True):
    ...
    for it in range(num_iter):
        ...
        mask = np.random.choice(N, batch_size)  # idee amelioration/test: peut essayer aussi de faire des partitions uniques à chaque itération
        X_batch = X_train[mask]
        y_batch = y_train[mask]
        scores = model.forward(X_batch)
        loss, grads, _ = model.calculate_loss(scores, y_batch, reg)
        ...
        # Périodiquement, on évalue l'accuracy d'entraînement et de
...
def sample(X, y, batch_size):
    """Permet de générer des batch d'entraînement pour une epoch.
        Cette fonction "découpe" les données dans X et y en une série
        de batches.
    Arguments:
...
class Adam:
    ...
    def reset(self):
        self.__init__(self.initial_lr,
                      ...
                      beta2=self.beta2,
                      eps=self.eps)
def epoch_solver(X_train, y_train, X_val, y_val, reg, optimizer, lr_decay=1.0, epochs=10, batch_size=100, verbose=True):
    """Boucle d'entraînement générale d'un modèle par epochs. La méthode de descente de gradient varie
       en fonction de l'optimizer.
    Arguments:
        X_train {ndarray} -- Données d'entraînement.
                             Shape (N, D)
    ...
    Keyword Arguments:
        lr_decay {float} -- Paramètre de contrôle pour la diminution du taux d'apprentissage.
                            (default: {1.0})
        epochs {int} -- Nombre d'epochs d'entraînement (default: {10})
        batch_size {int} -- Nombre de données d'entraînement passées au modèle par itération.
                            (default: {200})
        verbose {bool} -- (default: {True})
    Returns:
    ...
    for epoch in range(epochs):
        batches = sample(X_train, y_train, batch_size)
        i = 0
        for X_batch, y_batch in batches:
            output = model.forward(X_batch)
            loss, dScores, _ = model.calculate_loss(output, y_batch, reg)
            loss_history.append(loss)
            model.backward(dScores)
            optimizer.step()
            i += 1
            if verbose and i % 20 == 0:
                print('(batch %d / %d) loss: %f' % (i, len(batches), loss))
        optimizer.lr *= lr_decay
        train_accuracy = check_accuracy(X_train, y_train, batch_size, model)
        val_accuracy = check_accuracy(X_val, y_val, batch_size, model)
        train_accuracy_history.append(train_accuracy)
        val_accuracy_history.append(val_accuracy)
        if verbose:
            print('(epoch %d / %d) loss: %f, train_acc: %f, val_acc: %f' %
                  (epoch + 1, epochs, loss, train_accuracy, val_accuracy))
    return loss_history, train_accuracy_history, val_accuracy_history
